...
@Plugin
@HomeItemType(value = "Lamps", creationInfo = DeconzDimmableGroupLamp.deCONZCreationInfo.class)
public class DeconzDimmableGroupLamp extends HomeItemAdapter implements HomeItem {

	public static class deCONZCreationInfo implements AutoCreationInfo {
		static final String[] CREATION_EVENTS = { "deCONZ_lightgroup" };
		...
		public boolean canBeCreatedBy(Event e) {
			return e.isType("deCONZ_lightgroup");
		}

		@Override
		public String getCreationIdentification(Event e) {
			return String.format("deCONZ groupnlamp %s: \"%s\"", e.getAttribute(DE_CONZ_ID),
	...
	private static final String MODEL = ("<?xml version = \"1.0\"?> \n"
			...
			+ "  <Action Name=\"bright\" 	Method=\"bright\" />" + "  <Action Name=\"dim\" 	Method=\"dim\" />"
			+ "  <Action Name=\"dim1\" 	Method=\"dim1\" />" + "  <Action Name=\"dim2\" 	Method=\"dim2\" />"
			+ "  <Action Name=\"dim3\" 	Method=\"dim3\" />" + "  <Action Name=\"dim4\" 	Method=\"dim4\" />"
...
//			+ "  <Action Name=\"LearnDim4\" 	Method=\"learnDim4\" />"
	...
	public DeconzDimmableGroupLamp() {
	}
	...
	public boolean receiveEvent(Event event) {
		if (event.getAttribute(Event.EVENT_TYPE_ATTRIBUTE).equals("deCONZ_lightgroup")
			...
			String onAttribute = event.getAttribute("deCONZ.all_on");
	...
	private String getColorFromEvent(Event event) {
		int brightness = hueTopercent(event.getAttributeInt("deCONZ.Brightness"));
		if (event.hasAttribute("deCONZ.Hue")) {
...
//		setName(event.getAttribute("deCONZ.Name"));
	...
	protected void sendOnCommand(int brightness, int temperature, int hue, int saturation) {
		...
		ev.setAttribute("deCONZ.Brightness", percentToHue(brightness));
		if (temperature != 0) {
			ev.setAttribute("deCONZ.Temperature", temperature);
		} else if (saturation > 0) {
			ev.setAttribute("deCONZ.Saturation", saturation);
			ev.setAttribute("deCONZ.Hue", hue);
		}
		server.send(ev);
		isOn = true;
	}
	...
	private Event createEvent() {
		Event ev = server.createEvent("deCONZ_lightgroup", "");
